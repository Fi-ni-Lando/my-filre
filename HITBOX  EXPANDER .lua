...
local function createHitbox(player)
	if not shouldShow(player) or not player.Character then return end
...
local function onCharacterAdded(player, character)
	if not shouldShow(player) then return end
...
local function toggleSeeThrough()
	...
	for _, p in ipairs(Players:GetPlayers()) do
		if shouldShow(p) and p.Character then
...
local function resizeHitbox(smaller)
	local newSize
	if smaller then
		newSize = math.max(2, visualSize.X - sizeStep)
	else
		newSize = visualSize.X + sizeStep
	end
