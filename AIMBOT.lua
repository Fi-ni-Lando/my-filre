...
local A = {Enabled=true, Active=false, LockPart="Head", FOV=120}
...
UIS.InputBegan:Connect(function(i, g)
	...
	if i.UserInputType == Enum.UserInputType.MouseButton2 then
		...
		if target then -- Only activate if a valid target exists
			A.Active = true
			DisableMouse()
			Locked = target
			UpdateIndicator()
...
RS.RenderStepped:Connect(function()
	...
	-- Fully sticky: only aim if Locked is valid
	if A.Active and A.Enabled and Locked and Locked.Character then
