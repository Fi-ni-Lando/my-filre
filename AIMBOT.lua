...
FOV.Thickness = 1.5
FOV.Radius = A.FOV
FOV.Color = Color3.fromRGB(148,0,211) -- violet when idle
...
local function UpdateIndicator()
    ...
    -- green when active, violet otherwise
...
UIS.InputBegan:Connect(function(i, g)
    ...
    if i.UserInputType == Enum.UserInputType.MouseButton2 then
        Holding = true
        -- Only activate immediately if a valid target exists at press
...
RS.RenderStepped:Connect(function()
    -- follow mouse with circle and keep indicator positioned
    local mousePos = UIS:GetMouseLocation()
    ...
    if Holding and A.Enabled then
        local closest = ClosestInFOV()
        if closest then
            -- If we weren't active but a target appears while still holding, activate now
